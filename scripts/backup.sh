...
if [[ "$TAG" == "daily" ]]; then
	curl -s -X POST -H 'Content-Type: application/json' -d '{"text":"Daily backup completed!"}' $DAILY_HEALTHCHECKS_URL &>> "$LOG_FILE" \
		&& echo "Daily curl notification sent." | tee -a "$LOG_FILE"
elif [[ "$TAG" == "monthly" ]]; then
	curl -s -X POST -H 'Content-Type: application/json' -d '{"text":"Monthly backup completed!"}' $MONTHLY_HEALTHCHECKS_URL &>> "$LOG_FILE" \
